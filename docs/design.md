...
[Claude Code]
   ...
   ▼  stdin / JSON
[claude-logger 親プロセス]          … パース + 正規化のみ。即終了
   │  自分自身を --persist でデタッチ起動し、レコードJSONをstdinへ
   ▼
[claude-logger --persist 子プロセス] … バックグラウンドで永続化
   │
   ▼
...
6. **メッセージ・ファイルパス・引数・環境変数値などはすべて切り捨てる**。
   保存されるのは上記5項目のみで、機密情報は永続化層に到達しない。
7. クォート不整合などパース不能な入力はレコードを生成しない（安全側に倒す）。

### 正規化例

| 入力                                | base  | sub      | normalized   |
| ----------------------------------- | ----- | -------- | ------------ |
| `git commit -m "fix typo"`          | `git` | `commit` | `git commit` |
| `npm run dev --port 3000`           | `npm` | `run`    | `npm run`    |
| `ls -la /path/to/dir`               | `ls`  | ``       | `ls`         |
...
| `CSV_PATH`                                    | CSV出力先パス                          | `claude-logger.csv` |
